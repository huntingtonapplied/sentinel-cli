...
# Install the Sentinel CLI binary.
#
# This script is used by internal tooling (SEGA downloads publishing) and can also
# be used manually. It downloads the right binary for the current OS/arch and
# places it on PATH.

BIN_NAME="sentinel"
INSTALL_DIR_DEFAULT="/usr/local/bin"

# Default download base. Internal SEGA publishing typically serves:
#   ${BASE_URL}/cli/releases/<version>/sentinel-<os>-<arch>
BASE_URL_DEFAULT="${SENTINEL_DOWNLOADS_BASE_URL:-https://downloads.sentinelscanner.com}"
...
Options:
  ...
  --base-url     Base URL hosting /cli (default: $SENTINEL_DOWNLOADS_BASE_URL or https://downloads.sentinelscanner.com)
  --install-dir  Install directory (default: /usr/local/bin)

Environment:
  SENTINEL_DOWNLOADS_BASE_URL  Overrides default base URL
EOF
}
...
tmp="$(mktemp -t sentinel-cli.XXXXXX)"
...
# Offer to add install dir to PATH if not already there
