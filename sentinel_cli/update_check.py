"""Non-blocking version update check for the Sentinel CLI."""
...
from sentinel_cli.config import load_config, save_config
from sentinel_cli.log import logger

# Check at most once every 24 hours
CHECK_INTERVAL_HOURS = 24
PYPI_URL = "https://pypi.org/pypi/sentinel-cli/json"
...
class UpdateChecker:
    ...
    def start(self):
        ...
        if os.getenv("SENTINEL_NO_UPDATE_CHECK", ""):
            return
    ...
    def notify_if_outdated(self):
        ...
        import click
        click.echo(
            f"\nA new version of sentinel is available: {self.latest_version} "
            f"(current: {__version__}). "
            f"Run: pip install --upgrade sentinel-cli",
            err=True,
        )
